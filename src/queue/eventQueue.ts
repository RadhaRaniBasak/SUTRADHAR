...
export const QUEUE_NAME = "slack-events";

export function parseRedisConnectionOptions(): RedisOptions {
  return {
...
export const eventQueue = new Queue<SlackEventCallback>(QUEUE_NAME, {
...
export async function enqueueSlackEvent(payload: SlackEventCallback): Promise<void> {
  ...
  await eventQueue.add("event_callback", payload, { jobId: payload.event_id });
}
